...
process {
	...
	Write-Host 'Checking CVS Locations: '($CvsCities -join ', ')
	...
	while (-Not $Booked) {
		...
		$CVSRequest = @{
			Uri     = "https://www.cvs.com/immunizations/covid-19-vaccine.vaccine-status.$($State.ToLower()).json?vaccineinfo"
			Headers = @{
				"method"           = "GET"
				"authority"        = "www.cvs.com"
				"scheme"           = "https"
				"path"             = "/immunizations/covid-19-vaccine.vaccine-status.$($State.ToUpper()).json?vaccineinfo"
				"pragma"           = "no-cache"
		...
		$CVSContent.responsePayloadData.data.NC | ForEach-Object { 
		
			## Filter to only local cities
			if ($_.city.ToLower() -in $CvsCities.ToLower()) {
		...
		$WalgreensRequest = @{
			...
			Body        = "{`"serviceId`":`"99`",`"position`":{`"latitude`":34.236698,`"longitude`":-77.946198},`"appointmentAvailability`":{`"startDateTime`":`"$Tomorrow`"},`"radius`":25}"
		}
		...
		Start-Sleep -Seconds Math::Abs($SecondsToWait)
